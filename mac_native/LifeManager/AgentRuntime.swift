...
final class AgentRuntime {
    ...
    private func runCaptureLoop() async {
        while !Task.isCancelled {
            ...
            try? await Task.sleep(for: .seconds(Double(max(60, store.captureInterval))))
        }
    }
    ...
    private var currentAgentState: String {
        ...
        return currentPermissionsState == "ok" ? "running" : "degraded"
    }
    ...
    private func sendHeartbeat() async {
        ...
        do {
            ...
            if wasFailingBefore {
                notifier.deliver(kind: .callout, title: "Vero", body: "Connection to the backend has been restored.")
            }
            ...
            if consecutiveHeartbeatFailures == 3 {
                notifier.deliver(kind: .callout, title: "Vero", body: "The background agent has not reached the backend for several minutes.")
            }
        }
    }
